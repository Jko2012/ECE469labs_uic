module adder(
	...
	//use library implementation of adder
...
module subtractor(
	...
	//implement adder modules
	adder a0(a[0],x0,addnot,diff[0],c0);	//first adder inputs addnot as cin
	adder a1(a[1],x1,c0,diff[1],c1);
	adder a2(a[2],x2,c1,diff[2],c2);
	adder a3(a[3],x3,c2,diff[3],c3);
	adder a4(a[4],x4,c3,diff[4],c4);
	adder a5(a[5],x5,c4,diff[5],c5);
	adder a6(a[6],x6,c5,diff[6],c6);
	adder a7(a[7],x7,c6,diff[7],c7);
	adder a8(a[8],x8,c7,diff[8],c8);
	adder a9(a[9],x9,c8,diff[9],c9);
	adder a10(a[10],x10,c9,diff[10],c10);
	adder a11(a[11],x11,c10,diff[11],c11);
	adder a12(a[12],x12,c11,diff[12],c12);
	adder a13(a[13],x13,c12,diff[13],c13);
	adder a14(a[14],x14,c13,diff[14],c14);
	adder a15(a[15],x15,c14,diff[15],OF);	//last adder outputs cout as OF
endmodule
...
assign gi = Ai & Bi;
assign pi = Ai | Bi;
endmodule // bit_lvl_gp

module blk_lvl_gp(
	input logic gik, pik, gk_1j, pk_1j,
	output logic gij, pij);
assign gij = gik | (gk_1j & pik);
assign pij = pik & pk_1j;
endmodule // blk_lvl_gp

module blk_lvl_g(
	input logic gik, pik, gk_1j,
	output logic gij);
assign gij = gik | (gk_1j & pik);
endmodule // blk_lvl_g
...
// 2bit blk_lvl_gp calculation

blk_lvl_g gblk10(g[1], p[1], g[0], g10);
// 2bit blk_lvl_g calculation
...
// 4bit blk_lvl_gp calculation

blk_lvl_g gblk30(g32, p32, g10, g30);
// 4bit blk_lvl_g calculation

blk_lvl_gp gpblk158(g1512, p1512, g118, p118, g158, p158);
// 8bit blk_lvl_gp calculation

blk_lvl_g gblk07(g74, p74, g30, g70);
// 8bit blk_lvl_g calculation

blk_lvl_g gblk150(g158, p158, g70, g150);
// 16bit blk_lvl_g calculation

blk_lvl_g gblk110(g118, p118, g10, g110);
blk_lvl_g gblk50(g54, p54, g30, g50);
blk_lvl_g gblk90(g98, p98, g70, g90);
blk_lvl_g gblk130(g1312, p1312, g110, g130);
blk_lvl_g gblk20(g[2], p[2], g10, g20);
blk_lvl_g gblk40(g[4], p[4], g30, g40);
blk_lvl_g gblk60(g[6], p[6], g50, g60);
blk_lvl_g gblk80(g[8], p[8], g70, g80);
blk_lvl_g gblk100(g[10], p[10], g90, g100);
blk_lvl_g gblk120(g[12], p[12], g110, g120);
blk_lvl_g gblk140(g[14], p[14], g130, g140);
// final blk_lvl_g calculations
...
logic [15:0] MagD, DifBA, DifAB, MagA, MagB, MSum, SDif, SumAB; 
